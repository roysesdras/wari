...
if (isset($_GET['action'])) {
    ...
    try {
        ...
        switch ($_GET['action']) {
            ...
            case 'get_stats':
                try {
                    $stats = [
                        ...
                        // --- NOUVEAUX KPIs (Point 2) ---
                        'new_14j'          => $pdo->query("SELECT COUNT(*) FROM wari_users WHERE date_inscription >= DATE_SUB(NOW(), INTERVAL 14 DAY)")->fetchColumn(),
                        'total_sales'      => $pdo->query("SELECT COALESCE(SUM(montant), 0) FROM wari_payments WHERE statut = 'approved'")->fetchColumn(),
                        'daily_visits'     => $pdo->query("SELECT COUNT(*) FROM wari_audit WHERE action IN ('LOGIN_SUCCESS', 'AUTO_LOGIN') AND time >= DATE_SUB(NOW(), INTERVAL 24 HOUR)")->fetchColumn(),
                        'flux_expenses'    => $pdo->query("SELECT COALESCE(SUM(amount), 0) FROM wari_expenses")->fetchColumn(),

                        // --- DONNÉES HISTORIQUES (Graphiques) ---
                        'hist_users'       => $pdo->query("SELECT DATE(date_inscription) as day, COUNT(*) as count FROM wari_users WHERE date_inscription >= DATE_SUB(NOW(), INTERVAL 14 DAY) GROUP BY DATE(date_inscription) ORDER BY day ASC")->fetchAll(PDO::FETCH_ASSOC),
                        'hist_sales'       => $pdo->query("SELECT DATE(date_creation) as day, COALESCE(SUM(montant),0) as total FROM wari_payments WHERE statut = 'approved' AND date_creation >= DATE_SUB(NOW(), INTERVAL 14 DAY) GROUP BY DATE(date_creation) ORDER BY day ASC")->fetchAll(PDO::FETCH_ASSOC),
                        'hist_connections' => $pdo->query("SELECT DATE(time) as day, COUNT(DISTINCT user_id) as count FROM wari_audit WHERE action IN ('LOGIN_SUCCESS', 'AUTO_LOGIN') AND time >= DATE_SUB(NOW(), INTERVAL 14 DAY) GROUP BY day ORDER BY day ASC")->fetchAll(PDO::FETCH_ASSOC),
                        'hist_academy'     => $pdo->query("SELECT DATE(complete_le) as day, COUNT(DISTINCT user_id) as count FROM academy_progress WHERE complete_le >= DATE_SUB(NOW(), INTERVAL 14 DAY) GROUP BY day ORDER BY day ASC")->fetchAll(PDO::FETCH_ASSOC),

                        // --- TEMPS MOYEN (7J) ---
                        'avg_session_time' => $pdo->query("SELECT COALESCE(ROUND(AVG(TIMESTAMPDIFF(MINUTE, min_t, max_t))), 0) 
                            FROM (SELECT MIN(time) as min_t, MAX(time) as max_t FROM wari_audit 
                                  WHERE time >= DATE_SUB(NOW(), INTERVAL 7 DAY) AND user_id IS NOT NULL 
                                  GROUP BY user_id, DATE(time) HAVING COUNT(*) > 1) as s")->fetchColumn()
                    ];
            ...
            case 'get_monitoring':
                try {
                    // Dernières actions audités
                    $audit = $pdo->query("SELECT * FROM wari_audit ORDER BY time DESC LIMIT 20")->fetchAll(PDO::FETCH_ASSOC);
                    
                    // Lecture brute des dernières erreurs PHP (si le fichier existe)
                    $errors = [];
                    $logFile = 'logs/errors_' . date('Y-m-d') . '.log';
                    if (file_exists($logFile)) {
                        $lines = file($logFile);
                        $errors = array_slice($lines, -10);
                    }

                    jsonResponse(true, ['audit' => $audit, 'errors' => $errors]);
                } catch (Exception $e) {
                    jsonResponse(false, ['msg' => 'Erreur monitor: ' . $e->getMessage()]);
                }
...
<body>
    ...
    <?php else: ?>
        <div class="admin-wrap">
            <div class="topbar">
                ...
                <div class="topbar-right">
                    <div style="background:var(--gold-dim);border:1px solid var(--gold-border);color:var(--gold);padding:4px 12px;border-radius:8px;font-size:11px;font-weight:700;display:flex;align-items:center;gap:6px;">
                        <span style="opacity:0.6">RECETTES :</span> <span id="stat-sales-top">—</span>
                    </div>
            ...
            <div class="main-content">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card">
                        <div class="stat-label">Inscriptions (14j)</div>
                        <div class="stat-value gold" id="stat-new-14j">—</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Visites (24h)</div>
                        <div class="stat-value blue" id="stat-visits">—</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-label">Ventes Licences (Total)</div>
                        <div class="stat-value gold" id="stat-sales-card">—</div>
                    </div>
                    <div class="stat-card">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(280px, 1fr));gap:20px;margin-top:24px;margin-bottom:30px;">
                    <!-- Row 1 -->
                    <div style="background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:20px;">
                        <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:2px;margin-bottom:15px;font-weight:700;">Inscriptions (14j)</div>
                        <div style="position: relative; height: 150px; width: 100%;">
                            <canvas id="chartUsers"></canvas>
                        </div>
                    </div>
                    <div style="background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:20px;">
                        <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:2px;margin-bottom:15px;font-weight:700;">Volume Ventes (F)</div>
                        <div style="position: relative; height: 150px; width: 100%;">
                    ...
                    <!-- Row 2 -->
                    <div style="background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:20px;">
                        <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:2px;margin-bottom:15px;font-weight:700;">Connexions (Uniques)</div>
                        <div style="position: relative; height: 150px; width: 100%;">
                            <canvas id="chartConnections"></canvas>
                        </div>
                    </div>
                    <div style="background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:20px;">
                        <div style="font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:2px;margin-bottom:15px;font-weight:700;">Academy (Consultations)</div>
                        <div style="position: relative; height: 150px; width: 100%;">
                ...
                <div class="table-wrap">
                    ...
                    <?php 
                    $total_listed = count($users);
                    foreach ($users as $index => $user): 
                        $isHidden = $index >= 3 ? 'style="display:none;" class="user-row hidden-row"' : 'class="user-row"';
                ...
                <?php if ($total_listed > 3): ?>
                    <div style="text-align:center;margin-top:20px; margin-bottom:20px;">
                        <button id="toggleUsersBtn" class="btn-sm" style="background:var(--surface2);border:1px solid var(--border);color:var(--muted);padding:8px 20px;border-radius:10px;cursor:pointer;font-size:11px;" onclick="toggleUsers()">
                            Voir plus (<?= $total_listed - 3 ?> restants) ↓
                        </button>
                    </div>
                <?php endif; ?>
        ...
        <!-- LICENCES --> 
                <div class="section-header" style="margin-top:40px;cursor:pointer;" onclick="toggleLicences()">
                    <div class="section-title">🔑 Licences d'accès <span id="licence-toggle-icon" style="font-size:12px;opacity:0.5;margin-left:10px;">[ Afficher + ]</span></div>
    ...
    <script>
        ...
        async function refreshStats() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if(document.getElementById('stat-visits')) document.getElementById('stat-visits').innerText = s.daily_visits;
                    if(document.getElementById('stat-sales-card')) document.getElementById('stat-sales-card').innerText = parseInt(s.total_sales || 0).toLocaleString('fr-FR') + ' F';
                    if(document.getElementById('stat-avg-time')) document.getElementById('stat-avg-time').innerText = s.avg_session_time || '0';
                    
                    // Recettes (Barre supérieure)
                    if(document.getElementById('stat-sales-top')) {
                        document.getElementById('stat-sales-top').innerText = parseInt(s.total_sales || 0).toLocaleString('fr-FR') + ' F';
                    }
        ...
        let chartSales = null;
        let chartConn = null;
        let chartAcademy = null;

        function renderCharts(histUsers, histSales, histConn, histAcademy) {
            ...
            const ctxAcademy = document.getElementById('chartAcademy').getContext('2d');
        ...
        function toggleLicences() {
            ...
            const icon = document.getElementById('licence-toggle-icon');
            licencesExpanded = !licencesExpanded;
            
            container.style.display = licencesExpanded ? 'block' : 'none';
        ...
        async function fetchMonitoring() {
            try {
                const res = await fetch(`${BASE}?action=get_monitoring&csrf_token=${encodeURIComponent(CSRF_TOKEN)}`);
                const data = await res.json();
                ...
                if (data.success) {
                    ...
                    if (data.audit && data.audit.length > 0) {
                        html += `<div style="color:var(--gold);margin-bottom:10px;text-transform:uppercase;letter-spacing:2px;font-weight:bold;">Logs d'Activité</div>`;
                        data.audit.forEach(log => {
                            const time = log.time.split(' ')[1];
                            html += `<div style="margin-bottom:6px;border-bottom:1px solid rgba(255,255,255,0.03);padding-bottom:4px;">
                                <span style="color:var(--blue);font-size:11px;">[${log.time}]</span> 
                                <span style="color:#fff;font-weight:bold;margin-left:5px;">${log.action}</span> 
                                <span style="opacity:0.7;margin-left:5px;">by ${log.email || 'system'} ${log.user_id ? `(#${log.user_id})` : ''}</span>
                                <span style="color:var(--amber);margin-left:10px;font-size:11px;">🔌 IP: ${log.ip || 'N/A'}</span>
                                <div style="font-size:11px;opacity:0.5;margin-left:55px;">
                                    📝 ${log.details || 'Aucun détail supplémentaire'} <br>
                                    🌐 <span style="color:var(--muted)">Appareil: ${log.user_agent ? log.user_agent.substring(0, 80) + '...' : 'Inconnu'}</span>
                                </div>
                            </div>`;
                        });
                    }
                    ...
                    if (data.errors && data.errors.length > 0) {
                        html += `<div style="color:var(--red);margin-top:20px;margin-bottom:10px;text-transform:uppercase;letter-spacing:2px;font-weight:bold;">Alertes Système</div>`;
                        data.errors.forEach(err => {
                            html += `<div style="color:var(--red);opacity:0.8;margin-bottom:4px;">${err}</div>`;
                        });
                    }
